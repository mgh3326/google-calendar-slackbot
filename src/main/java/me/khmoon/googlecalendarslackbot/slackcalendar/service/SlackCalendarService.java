...
public class SlackCalendarService {
  ...
  public Reservation reserveCall(ReservationDetails details, DateTime dateTime) {
    CompletableFuture.runAsync(() -> reserve(details, dateTime), executor);
    Event event = calendarService.createEventWith(ReservationDateTime.of(dateTime.getFormattedDate(),
            dateTime.getFormattedStartTime(), dateTime.getFormattedEndTime()), details);
    return ReservationConverter.toReservation(event)
            .orElseThrow(InvalidEventException::new);
  }
